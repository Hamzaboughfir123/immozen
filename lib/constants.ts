 ...
 * propriétaires depuis les 5 sites ImmoZen vers l'API winwin-backend.
 * Chaque site (Marrakech, Casablanca, Rabat, Tanger, Agadir) définit ses
 * propres valeurs via variables d'environnement ; ce fichier ne change pas
 * d'un site à l'autre, seul le `.env.local` de chaque repo diffère.
 */
export const PROPERTY_LEAD_CONFIG = {
  /** Ville affichée à l'utilisateur (lecture seule) — jamais éditable. */
  city: process.env.NEXT_PUBLIC_CITY ?? "Marrakech",
  /** Domaine du site courant, envoyé comme `sourceSite` — seule source de
   * vérité pour la ville côté backend (jamais un champ saisi). */
  siteDomain:
    process.env.NEXT_PUBLIC_SITE_DOMAIN ?? SITE_URL.replace(/^https?:\/\/(www\.)?/, ""),
  /** URL de l'API centrale winwin-backend (identique pour les 5 sites). */
